...
def generate_qrcode():
    ...
    if content.get()=="":
        messagebox.showerror("QR code generator", "Give some text")
    else:
...
def save_qrcode():
    ...
    if(name!=None):
        ...
        messagebox.showinfo(title = 'Customisable QR code generator', message = "qrcode saved successfully")
...
generate=Button(top, text ="GENERATE",font=( 'calibre',15, "bold"),command=generate_qrcode ,fg="#13d675", bg="Black")
...
#OTHER BUTTONS
bg=Button(bottom,text="Background Color",command=lambda:color_change("bg") ,fg="#13d675", bg="Black")
bg.grid(row=1,column=0,pady=20)

fg=Button(bottom,text="Foreground Color",command=lambda:color_change("fg"),fg="#13d675", bg="Black")
fg.grid(row=1,column=1,pady=20)

save_=Button(bottom,text="Save",command=save_qrcode ,fg="#13d675", bg="Black")
